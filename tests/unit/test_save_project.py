...
def build() -> tuple[SaveProject, FakeProjectRepository, FakeArtifactStore]:
    repository = FakeProjectRepository()
    artifact_store = FakeArtifactStore()
    return SaveProject(repository, artifact_store, FixedClock()), repository, artifact_store


async def test_copies_every_artifact_into_the_artifact_store():
    use_case, _repository, artifact_store = build()
    result = make_pipeline_result()

    await use_case.execute(result)

    filenames = {filename for _source, _pid, filename in artifact_store.stored_files}
    assert filenames == {"scene.glb", "animation.mp4", "thumbnail.png"}


async def test_persists_under_the_businesss_own_id():
    use_case, repository, _artifact_store = build()
    result = make_pipeline_result()
...
async def test_auto_picks_a_before_image_when_a_photo_exists():
    use_case, _repository, _artifact_store = build()
...
async def test_persists_a_failed_pipeline_result_too():
    ...
    use_case, repository, artifact_store = build()
    ...
    assert artifact_store.stored_files == []
