...
**Phase 3 complete — AI design brief.** Discovery and enrichment work end to
end against live OpenStreetMap; a brief is then generated by Claude with
tool-forced structured output, constrained to the vocabulary the renderer
publishes, and exposed as `POST /brief` and `makeover brief`. Phase 2 delivered
the robots.txt gate, per-host rate limiter, httpx fetcher with a Playwright
fallback, JSON-LD + Open Graph extraction, and retention and redaction
policies; Google Places is wired in as an alternate directory behind a flag.

**Not verified against the live Anthropic API.** Every brief path is covered by
tests with a deterministic fake, and the capability handshake was checked
against a locally running `makeover-render`, but no real model call has been
made — that needs an API key this machine does not have.
